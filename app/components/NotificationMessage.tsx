    ...
    <AnimatePresence>
      {show ? (
        <motion.div
          ...
          className={clsx(
            'pointer-events-none fixed left-0 right-0 z-50 px-10',
            {
        ...
        >
          ...
          >
            <div className="bg-black text-white dark:bg-white dark:text-black pointer-events-auto relative max-w-xl rounded-lg p-8 pr-14 shadow-md">
              {typeof controlledVisible === 'undefined' ? (
                <button
                  aria-label="dismiss message"
                  onClick={() => setIsVisible(false)}
                  className="text-primary-light hover:text-black focus:text-inverse absolute right-4 top-8 rotate-45"
                >
                  <PlusIcon />
                </button>
              ) : null}
